                    ...
                    <?php if (!empty($roles)): ?>
                        <?php foreach ($roles as $role): ?>
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="text-sm font-medium text-gray-900 dark:text-gray-200">
                                            <a href="/roles/view/<?= htmlspecialchars($role->id) ?>" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 dark:text-gray-200">
                                        <?= !empty($role->created_at) ? date('M j, Y', strtotime($role->created_at)) : 'N/A' ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end space-x-3">
                                        <a 
                                            href="/roles/view/<?= htmlspecialchars($role->id) ?>" 
                                            class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300"
                                        >
                                            View
                                        </a>
                                        <a 
                                            href="/roles/edit/<?= htmlspecialchars($role->id) ?>" 
                                            class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300"
                                        >
                                            Edit
                                        </a>
                                        <form 
                                            action="/roles/delete/<?= htmlspecialchars($role->id) ?>" 
                                            method="POST" 
