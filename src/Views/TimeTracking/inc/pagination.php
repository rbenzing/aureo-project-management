<?php
//file: Views/time-tracking/inc/pagination.php
...
<div class="bg-white dark:bg-gray-800 px-4 py-3 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 sm:px-6 rounded-lg shadow mt-6">
    <div class="flex-1 flex justify-between sm:hidden">
        ...
        <?php if ($currentPage > 1): ?>
            <a href="<?= buildTimeTrackingPaginationUrl($currentPage - 1, $queryParams) ?>" 
    ...
    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
        <div>
            <p class="text-sm text-gray-700 dark:text-gray-300">
                Showing
                <span class="font-medium"><?= number_format($pagination['start']) ?></span>
                to
                <span class="font-medium"><?= number_format($pagination['end']) ?></span>
                of
        ...
        <div>
            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                ...
                <?php if ($startPage > 1): ?>
                    <a href="<?= buildTimeTrackingPaginationUrl(1, $queryParams) ?>" 
                ...
                <?php for ($page = $startPage; $page <= $endPage; $page++): ?>
                    ...
                    <?php else: ?>
                        <a href="<?= buildTimeTrackingPaginationUrl($page, $queryParams) ?>" 
                           class="relative inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
                            <?= $page ?>
                        </a>
                    <?php endif; ?>
                ...
                <?php if ($endPage < $totalPages): ?>
                    ...
                    <?php endif; ?>
                    <a href="<?= buildTimeTrackingPaginationUrl($totalPages, $queryParams) ?>" 
                       class="relative inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
                        <?= $totalPages ?>
                    </a>
                <?php endif; ?>
